...
private static List<Perceptron_node> f_arr = new ArrayList<Perceptron_node>(2);
	
	public static void face_train(ArrayList<Data_obj> data_arr, double train_per, double lRate) {
		...
		Perceptron_node [] temp_arr = f_arr.toArray(new Perceptron_node[2]);
		...
		f_arr = new ArrayList<Perceptron_node>(Arrays.asList(temp_arr));
	}
	...
	private static double matmul(Double [][] mat1,Double [][] mat2, Double tot) {
		...
		for(int i = 0; i < mat1.length; i++) {
			for(int j = 0; j < mat1[0].length; j++) {
				amt = Perceptron_node.setTot(mat2) - tot;
	...
	public static void val_check(ArrayList<Data_obj> data_arr, double lRate) {
		int guess = -1;
		Perceptron_node [] temp_arr = f_arr.toArray(new Perceptron_node[10]);
